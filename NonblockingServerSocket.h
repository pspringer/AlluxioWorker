...
#ifndef _THRIFT_TRANSPORT_NONBLOCKINGSERVERSOCKET_H_
#define _THRIFT_TRANSPORT_NONBLOCKINGSERVERSOCKET_H_ 1
 ...
 *
 */
class NonblockingServerSocket : public TServerTransport {
public:
   ...
   */
  NonblockingServerSocket(int port, int sendTimeout, int recvTimeout);
   ...
   * @param port    Port number to bind to
   */
  NonblockingServerSocket(const std::string& address, int port);
   ...
   */
  NonblockingServerSocket(const std::string& path);
...
#endif // #ifndef _THRIFT_TRANSPORT_NONBLOCKINGSERVERSOCKET_H_
