...
NonblockingServerSocket::NonblockingServerSocket(int port, int sendTimeout, int recvTimeout)
  : interruptableChildren_(true),
    port_(port),
    serverSocket_(THRIFT_INVALID_SOCKET),
    acceptBacklog_(DEFAULT_BACKLOG),
...
NonblockingServerSocket::NonblockingServerSocket(const string& address, int port)
  : interruptableChildren_(true),
    port_(port),
    address_(address),
    serverSocket_(THRIFT_INVALID_SOCKET),
...
NonblockingServerSocket::NonblockingServerSocket(const string& path)
  : interruptableChildren_(true),
    port_(0),
    path_(path),
    serverSocket_(THRIFT_INVALID_SOCKET),
...
void NonblockingServerSocket::setSendTimeout(int sendTimeout) {
  sendTimeout_ = sendTimeout;
}

void NonblockingServerSocket::setRecvTimeout(int recvTimeout) {
  recvTimeout_ = recvTimeout;
}

void NonblockingServerSocket::setAcceptTimeout(int accTimeout) {
  accTimeout_ = accTimeout;
}

void NonblockingServerSocket::setAcceptBacklog(int accBacklog) {
  acceptBacklog_ = accBacklog;
}

void NonblockingServerSocket::setRetryLimit(int retryLimit) {
  retryLimit_ = retryLimit;
}

void NonblockingServerSocket::setRetryDelay(int retryDelay) {
  retryDelay_ = retryDelay;
}

void NonblockingServerSocket::setTcpSendBuffer(int tcpSendBuffer) {
  tcpSendBuffer_ = tcpSendBuffer;
}

void NonblockingServerSocket::setTcpRecvBuffer(int tcpRecvBuffer) {
  tcpRecvBuffer_ = tcpRecvBuffer;
}

void NonblockingServerSocket::setInterruptableChildren(bool enable) {
  if (listening_) {
...
void NonblockingServerSocket::listen() {
  listening_ = true;
#ifdef _WIN32
...
int NonblockingServerSocket::getPort() {
  return port_;
}

shared_ptr<TTransport> NonblockingServerSocket::acceptImpl() {
  if (serverSocket_ == THRIFT_INVALID_SOCKET) {
  ...
  while (true)
     ...
     */
    // default value of -1 for accTimeout_ means block
    ...
    if (ret < 0) {
      ...
      {	// accTimeout_ timeout expired
    	// this might actually be a bug fix, because caller handles TIMED_OUT
//		  GlobalOutput("TServerSocket::acceptImpl() THRIFT_POLL 0");
//		  throw TTransportException(TTransportException::UNKNOWN);
		  throw TTransportException(TTransportException::TIMED_OUT);
      }
  }		// breaking out of the above loop means there is connection request
...
shared_ptr<TSocket> NonblockingServerSocket::createSocket(THRIFT_SOCKET clientSocket) {
...
void NonblockingServerSocket::notify(THRIFT_SOCKET notifySocket) {
...
void NonblockingServerSocket::interrupt() {
  notify(interruptSockWriter_);
}

void NonblockingServerSocket::interruptChildren() {
  notify(childInterruptSockWriter_);
}

void NonblockingServerSocket::close() {
  if (serverSocket_ != THRIFT_INVALID_SOCKET) {
